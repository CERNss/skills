...
**Priority 2: whisper.cpp speech-to-text on macOS Apple Silicon**

When running on macOS Apple Silicon, prefer `whisper-cli` with Metal acceleration.  
Do not switch to `openai-whisper`.  
Do not assume GPU failure only because Codex reports a memory allocation warning.  
If the same command works in the user's Terminal, treat the local whisper.cpp setup as valid.

Use the exact `whisper-cli` binary available in the user's shell when possible:
```bash
which whisper-cli
whisper-cli --help | head
```
...
```
yt-dlp --cookies-from-browser chrome -f "bestaudio/best" -o "audio.%(ext)s" "<URL>"
...
Chinese lecture transcription on macOS Apple Silicon:
...
```
Whisper 在 GPU 模式下显存分配失败了，我会切到 CPU 模式继续转写
```

do not immediately accept this as a real failure.
...
Then retry the same command the user successfully ran in Terminal.

If Metal/GPU transcription fails inside Codex but the same command works in the user's Terminal, treat it as likely sandbox-related. Before falling back to CPU, rerun the exact same `whisper-cli` command with escalated sandbox permissions so it runs outside the Codex seatbelt sandbox. Only use `-ng` CPU mode if the escalated GPU run also exits non-zero and no `transcript.srt` is produced.

Only fall back to CPU mode when `whisper-cli` actually exits with a non-zero status and no `transcript.srt` is produced.

After transcription, verify the output:

```
ls -lh transcript.srt transcript.txt
head -40 transcript.srt
```

The expected subtitle output is:

```
transcript.srt
```

Use `transcript.srt` as the timestamped subtitle source for later key-frame search and figure alignment.

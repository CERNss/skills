...
**这是一个分发壳，不是源头。** 真正的工具包模板维护在 skills 仓库的
`prompts/codex/anti-silent-fallback/`——本机 checkout 在哪，由环境变量
`FALLBACK_TOOLKIT_SOURCE` 告诉同步脚本，plugin 内不写死任何机器的绝对路径。

本 plugin 把那份工具包整份拷进 `assets/toolkit/`，再包三个 skill，让 codex
能在任意仓库里「接入 / 审计 / 自检」，不必手动去源头仓库里翻文件。

## 目录

```
.codex-plugin/plugin.json   plugin manifest
assets/toolkit/             工具包的完整拷贝（唯一正本在 skills 仓库，见下方同步说明）
scripts/sync-from-source.sh 从 skills 源头单向同步到 assets/toolkit/
skills/
...
## 同步方向（单向：skills → plugin）

改动一律改 skills 仓库 `prompts/codex/anti-silent-fallback/` 那份，然后跑：
...
它是 `rsync -a --delete` 从源头到 `assets/toolkit/`，
